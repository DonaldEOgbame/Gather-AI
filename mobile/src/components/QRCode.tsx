...
export function QRCode({ size = 132, seed = 0 }: { size?: number; seed?: number }) {
  ...
  return (
    <View style={{ width: size, height: size, backgroundColor: "#fff", borderRadius: 16, borderWidth: 1, borderColor: palette.border, padding: pad }}>
      <View style={{ flex: 1, flexDirection: "row", flexWrap: "wrap" }}>
        {Array.from({ length: N * N }, (_, i) => {
          ...
          return <View key={i} style={{ width: cell, height: cell, backgroundColor: on ? palette.text : "transparent", borderRadius: 1 }} />;
        })}
      </View>
    </View>
  );
