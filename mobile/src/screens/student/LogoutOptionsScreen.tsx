...
function Option({ icon, accent, title, sub, selected, onPress }: { icon: "lock" | "trash"; accent: "mint" | "peach"; title: string; sub: string; selected: boolean; onPress: () => void }) {
  const { palette } = useTheme();
  return (
    <Pressable onPress={onPress} style={{ backgroundColor: palette.card, borderRadius: 18, padding: 16, borderWidth: 1.5, borderColor: selected ? palette.text : palette.border, flexDirection: "row", alignItems: "center", gap: 11, shadowColor: palette.shadow, shadowOpacity: selected ? 0 : 0.05, shadowRadius: 3, shadowOffset: { width: 0, height: 1 }, elevation: selected ? 0 : 1 }}>
      ...
      <View style={{ width: 22, height: 22, borderRadius: 11, backgroundColor: selected ? palette.text : "transparent", borderWidth: selected ? 0 : 2, borderColor: palette.border, alignItems: "center", justifyContent: "center" }}>
        {selected ? <Icon name="check" size={13} color={palette.primaryText} width={2.6} /> : null}
      </View>
    </Pressable>
  );
}
